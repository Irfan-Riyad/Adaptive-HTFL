...
def run_single_experiment(scenario, strategy_label, config, partitions,
    ...
    for rnd in range(1, n_rounds + 1):
        ...
        if verbose:
            dpot_str = ""
            if dpot:
                c = "OK" if dpot["consensus"] else "NO"
...
def main(config=None, verbose=True):
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info(f"{'Scenario':<22} {'FedAvg':>9} {'BasicTrust':>11} {'HTFL':>9} {'Delta vs FedAvg':>15} {'Detect':>8} {'Compress':>9}")
    logger.info("=" * 80)
